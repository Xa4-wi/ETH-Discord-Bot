...
- Feature-specific debug data belongs in that feature, such as `/stats debug`.
...
Both commands are public and require only the ordinary permissions needed for
Cody to receive an interaction and send an embed. They expose no credentials,
private member data, or organizer-only diagnostics.
